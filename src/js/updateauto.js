...
(function(win, $) {
	...
	var keyUpdate = "updateCheck", //取消升级键名
		...
		checkInterval = 1 * 1000; //升级检查间隔，单位为ms,7天为7*24*60*60*1000=604800000, 如果每次启动需要检查设置值为0
	...
	function plusReady() {
		...
		//非wifi不自动更新
		if(currentNetwork != plus.networkinfo.CONNECTION_WIFI) {
			return;
		}
		...
		plus.runtime.getProperty(plus.runtime.appid, function(inf) {
			...
			//console.error(JSON.stringify(inf));

			currentVer = inf.version;

			if($.os.ios) {
				appVersionManager.ios.checkUpdate();
			} else {
				appVersionManager.android.checkUpdate();
			}
		});
	}
	...
	function successCallback(responseText) {
		...
		//console.error("successCallback" + responseText);
		...
		//  "createUser": 262,
		//  "deleteFlag": "0",
		//  "downloadUrl": "https://www.pgyer.com/FRh8,http://imgt.yinchengku.com/vsftp/app/1467945355664.wgt",
		//  "id": 73,
		//  "version": "1.7.4"
		//}
		//}
		...
		if(checkStatus == 0) {
			...
			//自动更新资源包文件，等待下次启动，自动更新
			//分－ios与android系统
			if($.os.ios) {
				// 下载升级包
				appVersionManager.ios.downWgt();
				...
				plus.nativeUI.confirm(upgradeObj.context, function(i) {
					if(0 == i.index) {
						//立即更新
						appVersionManager.android.downWgt();
					} else {
						//取　　消
		...
		ios: {
			...
			installWgt: function(path) {
				...
				plus.runtime.install(path, {}, function() {
					...
					console.log("安装资源包文件失败[" + e.code + "]：" + msg);
			...
			downWgt: function() {
				...
				function completedCB(download, status) {
					if(status == 200) {
						...
						appVersionManager.ios.installWgt(download.filename); // 安装wgt包
					}
				}
			}
		},
		android: {
			...
			downWgt: function() {
				...
				function completedCB(download, status) {
					...
					if(status == 200) {
						//console.log("下载升级包成功：" + download.filename);
						appVersionManager.android.installWgt(download.filename); // 安装wgt包
					} else {
						//console.log("下载升级包失败！");
						//plus.nativeUI.alert("下载升级包失败！");
